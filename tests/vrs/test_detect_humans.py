...
class TestCaseNormal(object):
    ...
    def test_ok_help(self, config, option):
        runner = CliRunner()
        result = runner.invoke(vrs.detect_humans, [option], obj=config)
        assert result.exit_code == 0
        assert 'Detect the humans of the requested image.' in result.output
...
class TestCaseException(object):
    ...
    def test_invalid_command(self, config, option):
        ...
        assert 'Usage: detect_humans [OPTIONS]' in result.output
