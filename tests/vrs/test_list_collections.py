...
class TestCaseNormal(object):
    ...
    def test_ok_help(self, config, option):
        runner = CliRunner()
        result = runner.invoke(vrs.list_collections, [option], obj=config)
        assert result.exit_code == 0
        assert 'List face collections.' in result.output

    def test_ok_list_collections(self, config):
        runner = CliRunner()
        result = runner.invoke(vrs.list_collections, obj=config)
        assert result.exit_code == 0
        assert 'face_collections' in result.output
