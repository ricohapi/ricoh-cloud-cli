...
from ricohcloudcli.ips.filter.types import median as commands
from . import util

INVALID_KSIZE = 'Invalid value for "-k" / "--ksize"'
...
class TestCaseNormal(object):
    ...
    def test_ok_help(self, config, option):
        runner = CliRunner()
        result = runner.invoke(commands.median, [option], obj=config)
        assert result.exit_code == 0
        assert 'Applies image filters to an image using a median filter, to standard output.' in result.output
    ...
    def test_ok_input(self, config, key, value):
        runner = CliRunner()
        options = [key, value, '-l', '0,0,10,10', '-k', 3]
        result = runner.invoke(commands.median, options, obj=config)
    ...
    def test_ok_output(self, config, key, value):
        runner = CliRunner()
        options = ['-i', util.TEST_IMAGE_BINARY_JPG, '-l', '0,0,10,10', '-k', 3]
        if key:
...
class TestCaseException(object):
    ...
    def test_invalid_command(self, config, option):
        ...
        assert 'Usage: median [OPTIONS]' in result.output
    ...
    def test_input_missing_path(self, config, key, value):
        runner = CliRunner()
        options = [key, value, '--location', '0,0,10,10', '-k', 3]
        result = runner.invoke(commands.median, options, obj=config)
    ...
    def test_output_invalid_format(self, config, key, value):
        runner = CliRunner()
        options = ['-i', util.TEST_IMAGE_BINARY_JPG, '--location', '0,0,10,10', '-k', 3, key, value]
        result = runner.invoke(commands.median, options, obj=config)
    ...
    def test_location_invalid_format(self, config, key, value):
        runner = CliRunner()
        options = ['-i', util.TEST_IMAGE_BINARY_JPG, key, value, '-k', 3]
        result = runner.invoke(commands.median, options, obj=config)
        assert result.exit_code == 2, result.output
